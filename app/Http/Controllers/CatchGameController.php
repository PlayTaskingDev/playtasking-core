...
class CatchGameController extends Controller
{
    ...
    public function show($slug)
    {
        ...
        $catch_game = CatchGame::with('campaign','catch_objects')
            ->where(
            [
                ['slug', $slug],
                ['init_date','<',$today],
        ...
        $has_paticipated = $this->check_participation($model_id = $catch_game->id,$model_type = 'App\Models\CatchGame',$user_id = Auth::user()->id);
    ...
    {
        $rules = [
            'data' => ['required','exists:catch_games,id']
        ];
        ...
        $catch_game = CatchGame::with('award')->findOrFail($data['data']);
        ...
        $query = DB::table('award_user')->insert([
            ...
            'model_type'    => 'App\Models\CatchGame',
            'hit'           => true,
