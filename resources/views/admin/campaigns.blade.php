                ...
                <tbody>
                    @foreach ($campaigns as $campaign)
                        ...
                        <div class="flex items-center justify-start space-x-3">
                            ...
                            <form method="post" action="{{ route('campaigns.destroy', ['tenant' => tenant('id'), 'campaign' => $campaign]) }}">
                                @csrf
                                @method('delete')
        ...
        <form id="form-campaign" method="POST" enctype="multipart/form-data">
            <div class="px-2 overflow-y-auto custom-scrollbar h-[510px]">
                <div class="grid grid-cols-1 gap-x-6 gap-y-5 lg:grid-cols-2">
                    ...
                    <x-ui.forms.input-datetime label="{{ __('Select Start Date') }}" name="init_date" placeholder="{{ __('Start Date') }}" value="" data-field="campaign.init_date"  data-type="datetime"/>
                    <x-ui.forms.input-datetime label="{{ __('Select End Date') }}" name="end_date" placeholder="{{ __('End Date') }}" value="" data-field="campaign.end_date"  data-type="datetime"/>
...
@vite(['resources/js/cruds/campaign.js'])
<script>
