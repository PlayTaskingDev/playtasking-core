...
<x-slot name="title">
    {{ $catch_object->id == null ? trans('Create') . ' ' . trans('Object') : $catch_object->name }}
</x-slot>
...
<x-slot name="header">
    ...
    {{ $catch_object->id == null ? trans('Create') : trans('Edit') }} {{ __('Object') }}
    </h1>
</x-slot>
...
@section('content')
    <div class="py-6 mx-5">
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8 bg-white p-3 rounded shadow">
            ...
            action="{{ $catch_object->id == null ? route('catchgameobjects.store', ['tenant' => tenant('id')]) : route('catchgameobjects.update', ['tenant' => tenant('id'), 'catchgameobject' => $catch_object]) }}">
            @csrf
            @isset($catch_object->id)
            @method('PATCH')
            <input type="hidden" name="id" value="{{ $catch_object->id }}">
            @endisset

            <input type="hidden" name="catch_game_id" value="{{ !is_null($catch_object->id) ? $catch_object->catch_game_id : $catch_game_id }}">

            <div class="my-5">
                <x-input-label for="object_image" :value="__('Object Image')" />
                @if (!is_null($catch_object->id) && $catch_object->object_image)
                    <img src="{{$catch_object->object_image}}" alt="{{__('Object Image')}}" title="{{__('Object Image')}}" class="my-5">
                @endif
                <input
            ...
            <div class="flex my-5">
                ...
                @if (!is_null($catch_object->id))
                    <a href="{{ route('catchgameobjects.edit', ['tenant' => tenant('id'), 'catchgameobject' => $catch_object]) }}"
