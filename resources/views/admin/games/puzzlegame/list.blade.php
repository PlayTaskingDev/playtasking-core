...
@section('content')
    @if (session('status'))
        ...
        />
    @endif
    <x-v2.common.page-breadcrumb pageTitle="{{ $title }}" desc="Aquí podras agregar y editar juegos tipo Rompecabezas." isBtn='1' :titleBtn="__('Add PuzzleGame +')" routeBtn="{{ route('puzzlegames.create', ['tenant' => tenant('id')]) }}"/>
    <div class="space-y-6">
                ...
                <tbody>
                    @foreach ($puzzles as $puzzle)
                        ...
                        <div class="flex items-center gap-3">
                            <div class="w-64">
                                <span class="block font-bold text-gray-800 text-theme-sm dark:text-white/90" >{{ $puzzle->title }}</span>
                                <span class="block text-gray-500 text-theme-xs dark:text-gray-400" >{{ $puzzle->description }}</span>
                            </div>
                        </div>
                        </td>
