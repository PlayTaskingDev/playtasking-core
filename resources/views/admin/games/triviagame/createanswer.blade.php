...
<x-slot name="title">
    {{ $answer->id == null ? trans('Create') . ' ' . trans('Object') : $answer->name }}
</x-slot>
<x-slot name="description">
    {{ $answer->id == null ? '' : $answer->name }}
</x-slot>

<x-slot name="header">
    <h1 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
    {{ $answer->id == null ? trans('Create') : trans('Edit') }} {{ __('Object') }}
    </h1>
</x-slot>
...
@section('content')
    <div class="py-6 mx-5">
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8 bg-white p-3 rounded shadow">
            ...
            action="{{ $answer->id == null ? route('triviagameanswers.store', ['tenant' => tenant('id')]) : route('triviagameanswers.update', ['tenant' => tenant('id'), 'triviagameanswer' => $answer]) }}">
            @csrf
