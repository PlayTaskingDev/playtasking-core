...
@section('content')
    <div class="p-4 mx-auto max-w-(--breakpoint-2xl) md:p-6">
        ...
        <div class="space-y-6">
            ...
            action="{{ $quiz->id == null ? route('triviagames.store', ['tenant' => tenant('id')]) : route('triviagames.update', ['tenant' => tenant('id'), 'triviagame' => $quiz]) }}">
                    ...
                    <div class="rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-white/3">
                        <div class="px-2 overflow-y-auto ">
                            <div class="grid grid-cols-1 gap-x-6 gap-y-5 lg:grid-cols-2">
                                ...
                                <x-ui.forms.input-file label="{{ __('Top Banner') }}" dummy_img="/storage/dummy_assets/600x200.png" name="failed_image" placeholder="" :value="$quiz->game_banner" data-field="campaign.game_banner" />
                                <x-ui.forms.input-text label="{{ __('Banner URL (Image)') }}" name="game_banner_url" placeholder="" :value="$quiz->game_banner_url" data-field="campaign.game_banner_url" />
                                ...
                                <div id="question_container" class="py-6 mx-5 col-span-2">
                                    <div class="flex justify-between">
                                        ...
                                        {{ __('Questions') }}</h2>
                                        <a href="{{ route('triviagamequestions.create', ['tenant' => tenant('id'), 'quiz_id' => $quiz]) }}"
                                        ...
                                        <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                                            ...
                                            <tbody>
                                                @foreach ($quiz->questions as $question)
                                                    ...
                                                    <td class="px-6 py-4">
                                                    <a href="{{ route('triviagamequestions.edit', ['tenant' => tenant('id'), 'triviagamequestion' => $question]) }}"
                        ...
                        <div class="rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-white/3 mt-6">
                            <div class="flex justify-between">
                                ...
                                @if (is_null($quiz->award))
                                    <a href="{{ route('v2awards.create', ['tenant' => tenant('id'), 'awardable_id' => $quiz, 'awardable_type' => 'App\Models\Quiz' ]) }}"
                                    class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 mb-5 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800">
                                    {{ __('Create') }} {{ __('Award') }}
                                    </a>
                                @endif
                            ...
                            @if (!is_null($quiz->award))
                                <div class="relative overflow-x-auto shadow-md rounded-lg">
                                    ...
                                    <tbody>
                                        ...
                                        <td class="px-6 py-4">
                                        <a href="{{ route('v2awards.edit', ['tenant' => tenant('id'), 'v2award' => $quiz->award]) }}"
                                        class="font-medium text-blue-600 dark:text-blue-500 hover:underline">{{ __('Edit') }}</a>
                                        </td>
                                        </tr>
                                    </tbody>
            ...
            <div class="space-y-6 lg:col-span-4 2xl:col-span-3">
                <div class="rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-white/3">
                    ...
                    <x-ui.forms.input-datetime label="{{ __('Select Start Date') }}" iddatepicker="datepickerInitDate" class="mb-3" name="init_date" placeholder="{{ __('Start Date') }}" :value="$quiz->init_date" data-field="campaign.init_date"/>
                    <x-ui.forms.input-datetime label="{{ __('Select End Date') }}" iddatepicker="datepickerEndDate" name="end_date" placeholder="{{ __('End Date') }}" :value="$quiz->end_date" data-field="campaign.end_date"/>
                </div>
                ...
                <div class="rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-white/3">
                    ...
                    <x-ui.forms.input-color label="{{ __('Gradient Background 1') }}" name="gradient_1" placeholder="" :value="$quiz->gradient_1" data-field="campaign.gradient_1" />
                    <x-ui.forms.input-color label="{{ __('Gradient Background 2') }}" name="gradient_2" placeholder="" :value="$quiz->gradient_2" data-field="campaign.gradient_2" />
                </div>
            </div>
        </div>
    </form>
