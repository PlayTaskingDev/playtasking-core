...
@section('content')
    <div class="p-4 mx-auto max-w-(--breakpoint-2xl) md:p-6">
        ...
        <div class="space-y-6">
            ...
            action="{{ $penalGame->id == null ? route('penalgames.store', ['tenant' => tenant('id')]) : route('penalgames.update', ['tenant' => tenant('id'), 'penalgame' => $penalGame]) }}">
            <div class="mb-6 flex flex-col justify-between gap-6 rounded-2xl border border-gray-200 bg-white px-6 py-5 sm:flex-row sm:items-center dark:border-gray-800 dark:bg-white/3">
                <div class="flex flex-col gap-2.5 divide-gray-300 sm:flex-row sm:divide-x dark:divide-gray-700">
                    ...
                    @if (isset($penalGame->is_expired) && $penalGame->is_expired)
                        <p class="text-sm text-gray-500 sm:pl-3 dark:text-gray-400">Expires At:&nbsp;<strong>{{ $penalGame->only_date }}</strong></p>
                    @endif
                </div>
                    ...
                    <div class="rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-white/3">
                        <div class="px-2 overflow-y-auto ">
                            <div class="grid grid-cols-1 gap-x-6 gap-y-5 lg:grid-cols-2">
                                ...
                                @isset($penalGame->id)
                                @method('PATCH')
                                <input type="hidden" name="id" value="{{ $penalGame->id }}">
                                @endisset
                                ...
                                <x-ui.forms.input-select label="{{ __('Campaign') }}" :options="$campaigns" name="campaign_id" placeholder="" :value="($penalGame->campaign->id ?? null)" data-field="campaign.campaign_id" />
                                <x-ui.forms.input-text label="{{ __('Title') }}" name="title" placeholder="" :value="$penalGame->title" data-field="campaign.title" />
                                <x-ui.forms.input-text label="{{ __('Description') }}" cols="2" name="description" placeholder="" :value="$penalGame->description" data-field="campaign.description" /><x-ui.forms.input-file label="{{ __('Image On') }}" dummy_img="/storage/dummy_assets/600x200.png" name="featured_image" placeholder="" :value="$penalGame->featured_image" data-field="campaign.featured_image" />
                                <x-ui.forms.input-file label="{{ __('Image Off') }}" dummy_img="/storage/dummy_assets/600x200.png" name="featured_image_disabled" placeholder="" :value="$penalGame->featured_image_disabled" data-field="campaign.featured_image_disabled" />
                    ...
                    @if (!is_null($penalGame->id))
                        <div class="rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-white/3 mt-6">
                            <div class="flex justify-between">
                                ...
                                @if (is_null($penalGame->award))
                                    <a href="{{ route('v2awards.create', ['tenant' => tenant('id'), 'awardable_id' => $penalGame->id, 'awardable_type' => 'App\Models\PenalGame' ]) }}"
                            ...
                            @if (!is_null($penalGame->award))
                                <div class="relative overflow-x-auto shadow-md rounded-lg">
                                    ...
                                    <tbody>
                                        ...
                                        {!!$penalGame->award->title!!}
                                        </th>
                                        <td class="px-6 py-4">
                                        <a href="{{ route('v2awards.edit', ['tenant' => tenant('id'), 'v2award' => $penalGame->award]) }}"
            ...
            <div class="space-y-6 lg:col-span-4 2xl:col-span-3">
                <div class="rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-white/3">
                    ...
                    <x-ui.forms.input-datetime label="{{ __('Select Start Date') }}" iddatepicker="datepickerInitDate" class="mb-3" name="init_date" placeholder="{{ __('Start Date') }}" :value="$penalGame->init_date" data-field="campaign.init_date"/>
                    <x-ui.forms.input-datetime label="{{ __('Select End Date') }}" iddatepicker="datepickerEndDate" name="end_date" placeholder="{{ __('End Date') }}" :value="$penalGame->end_date" data-field="campaign.end_date"/>
                </div>
                <div class="rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-white/3 space-y-3">
                    ...
                    <x-ui.forms.input-text label="{{ __('Text Active') }}" name="btn_text_active" placeholder="" :value="$penalGame->btn_text_active" data-field="campaign.btn_text_active" />
                    <x-ui.forms.input-switch label="{{ __('Enable Button Shadow') }}" name="btn_enable_shadow" placeholder="" :value="$penalGame->btn_enable_shadow" data-field="campaign.btn_enable_shadow" />
                    <x-ui.forms.input-text label="{{ __('Text Inactive') }}" name="btn_text_inactive" placeholder="" :value="$penalGame->btn_text_inactive" data-field="campaign.btn_text_inactive" />
                </div>
                <div class="rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-white/3">
                    <h2 class="mb-5 text-lg font-semibold text-gray-800 dark:text-white/90">Card Settings</h2>
                    <x-ui.forms.input-color label="{{ __('Gradient Background 1') }}" name="gradient_1" placeholder="" :value="$penalGame->gradient_1" data-field="campaign.gradient_1" />
                    <x-ui.forms.input-color label="{{ __('Gradient Background 2') }}" name="gradient_2" placeholder="" :value="$penalGame->gradient_2" data-field="campaign.gradient_2" />
                </div>
            </div>
        </div>
    </form>
