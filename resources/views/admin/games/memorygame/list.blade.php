...
@section('content')
    @if (session('status'))
        ...
        />
    @endif
    <x-v2.common.page-breadcrumb pageTitle="{{ $title }}" desc="Aquí podras agregar y editar juegos tipo Memorama." isBtn='1' :titleBtn="__('Add MemoryGame +')" routeBtn="{{ route('memorygames.create', ['tenant' => tenant('id')]) }}" />
    <div class="space-y-6">
                ...
                <tbody>
                    @foreach ($memory_quizzes as $memory_quiz)
                        ...
                        <div class="flex items-center gap-3">
                            <div class="w-64">
                                <span class="block font-bold text-gray-800 text-theme-sm dark:text-white/90" >{{ $memory_quiz->title }}</span>
                                <span class="block text-gray-500 text-theme-xs dark:text-gray-400" >{{ $memory_quiz->description }}</span>
                            </div>
                        </div>
                        </td>
                        ...
                        <div class="flex items-center justify-end space-x-3">
                            ...
                            {{ __('Edit Award') }}</a>
                            <a href="{{ route('panel.export_user_interactions', ['tenant' => tenant('id'), 'table_name' => $memory_quiz->table_name, 'model_id' => $memory_quiz->id]) }}"
