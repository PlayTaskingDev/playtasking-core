...
@section('content')
    @if (session('status'))
        ...
        />
    @endif
    <x-v2.common.page-breadcrumb pageTitle="{{ $title }}" desc="Aquí podras agregar y editar juegos tipo Comparte y Gana." isBtn='1' :titleBtn="__('Add ShareGame +')" routeBtn="{{ route('sharegames.create', ['tenant' => tenant('id')]) }}" />
    <div class="space-y-6">
                ...
                <tbody>
                    @foreach ($share_quizzes as $share_quizze)
                        ...
                        <div class="flex items-center gap-3">
                            <div class="">
                                <span class="block font-bold text-gray-800 text-theme-sm dark:text-white/90" >{{ $share_quizze->title }}</span>
                                <span class="block text-gray-500 text-theme-xs dark:text-gray-400" >{{ $share_quizze->description }}</span>
                            </div>
                        </div>
                        </td>
