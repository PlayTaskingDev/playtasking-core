...
<x-slot name="title">
    {{ !is_null($flappyGame->title) ? $flappyGame->title : trans('Create') . '' . trans('Flappy Game') }}
</x-slot>
<x-slot name="description">
    {{ $flappyGame->id == null ? '' : $flappyGame->description }}
</x-slot>
<x-slot name="header">
...
@section('content')
    <div class="p-4 mx-auto max-w-(--breakpoint-2xl) md:p-6">
        <div class="flex flex-wrap items-center justify-between gap-3 mb-6">
            ...
            {{ $flappyGame->id == null ? trans('Create') : trans('Edit') }} {{ __('Flappy Game') }}
            </h2>
            <nav>
        ...
        <div class="space-y-6">
            ...
            action="{{ $flappyGame->id == null ? route('flappygames.store', ['tenant' => tenant('id')]) : route('flappygames.update', ['tenant' => tenant('id'), 'flappygame' => $flappyGame]) }}">
            <div class="mb-6 flex flex-col justify-between gap-6 rounded-2xl border border-gray-200 bg-white px-6 py-5 sm:flex-row sm:items-center dark:border-gray-800 dark:bg-white/3">
                <div class="flex flex-col gap-2.5 divide-gray-300 sm:flex-row sm:divide-x dark:divide-gray-700">
                    ...
                    @if (isset($flappyGame->is_expired) && $flappyGame->is_expired)
                        <p class="text-sm text-gray-500 sm:pl-3 dark:text-gray-400">Expires At:&nbsp;<strong>{{ $flappyGame->only_date }}</strong></p>
                    @endif
                </div>
                    ...
                    <div class="rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-white/3">
                        <div class="px-2 overflow-y-auto ">
                            <div class="grid grid-cols-1 gap-x-6 gap-y-5 lg:grid-cols-2">
                                ...
                                @isset($flappyGame->id)
                                @method('PATCH')
                                <input type="hidden" name="id" value="{{ $flappyGame->id }}">
                                @endisset
                                ...
                                <x-ui.forms.input-select label="{{ __('Campaign') }}" :options="$campaigns" name="campaign_id" placeholder="" :value="($flappyGame->campaign->id ?? null)" data-field="campaign.campaign_id" />
                                <x-ui.forms.input-text label="{{ __('Title') }}" name="title" placeholder="" :value="$flappyGame->title" data-field="campaign.title" />
                                <x-ui.forms.input-text label="{{ __('Description') }}" cols="2" name="description" placeholder="" :value="$flappyGame->description" data-field="campaign.description" /><x-ui.forms.input-file label="{{ __('Image On') }}" dummy_img="/storage/dummy_assets/600x200.png" name="featured_image" placeholder="" :value="$flappyGame->featured_image" data-field="campaign.featured_image" />
                                <x-ui.forms.input-file label="{{ __('Image Off') }}" dummy_img="/storage/dummy_assets/600x200.png" name="featured_image_disabled" placeholder="" :value="$flappyGame->featured_image_disabled" data-field="campaign.featured_image_disabled" />
                                <h2 class="mt-6 text-lg col-span-2 font-semibold text-gray-800 dark:text-white/90">Top Banner Settings</h2>
                                <x-ui.forms.input-file label="{{ __('Top Banner') }}" dummy_img="/storage/dummy_assets/600x200.png" name="game_banner" placeholder="" :value="$flappyGame->game_banner" data-field="campaign.game_banner" />
                                <x-ui.forms.input-text label="{{ __('Banner URL (Image)') }}" name="game_banner_url" placeholder="" :value="$flappyGame->game_banner_url" data-field="campaign.game_banner_url" />
            ...
            <div class="space-y-6 lg:col-span-4 2xl:col-span-3">
                <div class="rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-white/3">
                    ...
                    <x-ui.forms.input-datetime label="{{ __('Select Start Date') }}" iddatepicker="datepickerInitDate" class="mb-3" name="init_date" placeholder="{{ __('Start Date') }}" :value="$flappyGame->init_date" data-field="campaign.init_date"/>
                    <x-ui.forms.input-datetime label="{{ __('Select End Date') }}" iddatepicker="datepickerEndDate" name="end_date" placeholder="{{ __('End Date') }}" :value="$flappyGame->end_date" data-field="campaign.end_date"/>
                </div>
                <div class="rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-white/3 space-y-3">
                    ...
                    <x-ui.forms.input-color label="{{ __('Gradient Button Background 1') }}" name="btn_background_color_1" placeholder="" :value="$flappyGame->btn_background_color_1" data-field="campaign.btn_background_color_1" />
                    <x-ui.forms.input-color label="{{ __('Gradient Button Background 2') }}" name="btn_background_color_2" placeholder="" :value="$flappyGame->btn_background_color_2" data-field="campaign.btn_background_color_2" />
                    <x-ui.forms.input-color label="{{ __('Button Border Color') }}" name="btn_border_color" placeholder="" :value="$flappyGame->btn_border_color" data-field="campaign.btn_border_color" />
                    <x-ui.forms.input-color label="{{ __('Button Text Color') }}" name="btn_text_color" placeholder="" :value="$flappyGame->btn_text_color" data-field="campaign.btn_text_color" />
                    <x-ui.forms.input-switch label="{{ __('Has shadow') }}" name="btn_shadow" placeholder="" :value="$flappyGame->btn_shadow" data-field="campaign.btn_shadow" />
                    <x-ui.forms.input-text label="{{ __('Text Active') }}" name="btn_text_active" placeholder="" :value="$flappyGame->btn_text_active" data-field="campaign.btn_text_active" />
                    <x-ui.forms.input-switch label="{{ __('Enable Button Shadow') }}" name="btn_enable_shadow" placeholder="" :value="$flappyGame->btn_enable_shadow" data-field="campaign.btn_enable_shadow" />
                    <x-ui.forms.input-text label="{{ __('Text Inactive') }}" name="btn_text_inactive" placeholder="" :value="$flappyGame->btn_text_inactive" data-field="campaign.btn_text_inactive" />
                </div>
                <div class="rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-white/3">
                    <h2 class="mb-5 text-lg font-semibold text-gray-800 dark:text-white/90">Card Settings</h2>
                    <x-ui.forms.input-color label="{{ __('Gradient Background 1') }}" name="gradient_1" placeholder="" :value="$flappyGame->gradient_1" data-field="campaign.gradient_1" />
                    <x-ui.forms.input-color label="{{ __('Gradient Background 2') }}" name="gradient_2" placeholder="" :value="$flappyGame->gradient_2" data-field="campaign.gradient_2" />
                </div>
            </div>
        </div>
    </form>
