...
@section('content')
    @if (session('status'))
        ...
        />
    @endif
    <x-v2.common.page-breadcrumb pageTitle="{{ $title }}" desc="Aquí podras agregar y editar juegos tipo Aplazo." isBtn='1' :titleBtn="__('Add AplazoGame +')" routeBtn="{{ route('aplazogames.create', ['tenant' => tenant('id')]) }}" />
    <div class="space-y-6">
                ...
                <tbody>
                    @foreach ($aplazo_games as $aplazo_game)
                        ...
                        <div class="flex items-center gap-3">
                            <div class="w-64">
                                <span class="block font-bold text-gray-800 text-theme-sm dark:text-white/90" >{{ $aplazo_game->title }}</span>
                                <span class="block text-gray-500 text-theme-xs dark:text-gray-400" >{{ $aplazo_game->description }}</span>
                            </div>
                        </div>
                        </td>
                        ...
                        <div class="flex items-center justify-end space-x-3">
                            ...
                            @if($aplazo_game->award)
                            <a href="{{ route('v2awards.edit', ['tenant' => tenant('id'), 'v2award' => $aplazo_game->award]) }}" class=" inline-flex items-center gap-2 px-3 py-2 text-sm font-medium text-white bg-green-600 hover:bg-green-700 dark:bg-green-700 dark:hover:bg-green-800 rounded-lg transition-colors">
                            <x-heroicon-o-gift class="w-5"/>
                            {{ __('Edit Award') }}</a>
                            <a href="{{ route('panel.export_user_interactions', ['tenant' => tenant('id'), 'table_name' => $aplazo_game->table_name, 'model_id' => $aplazo_game->id]) }}"
