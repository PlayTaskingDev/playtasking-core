...
@section('content')
    ...
    <div class="space-y-6">
        ...
        <div class="w-full flex justify-end">
            <button
            data-action="create"
            data-modal-target="smashgame-modal"
            data-modal-toggle="smashgame-modal"
            data-save-route="{{ route('smashgames.store', tenant('id')) }}"
            class="btn bg-brand-500 hover:bg-brand-600 flex w-full justify-center rounded-lg px-4 py-2.5 text-sm font-medium text-white sm:w-auto" aria-label="{{ __('Add new SmashGame') }}">
            {{ __('Add SmashGame +') }}
            </button>
        </div>
                ...
                <tbody>
                    @foreach ($smash_games as $smash_game)
                        ...
                        <div class="flex items-center gap-3">
                            <div class="w-lg">
                                <span class="block font-medium text-gray-800 text-theme-sm dark:text-white/90" >{{ $smash_game->title }}</span>
                                <span class="block text-gray-500 text-theme-sm dark:text-gray-400" >{{ $smash_game->description }}</span>
                            </div>
                        </div>
                        </td>
