        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="relative overflow-x-auto rounded-lg mx-5">
                ...
                <div class="grid justify-items-end">
                    <a href="{{ route('memory_quizzes.create', ['tenant' => tenant('id')]) }}"
                        ...
                        {{ __('Create') }} {{ __('Memory Quiz') }}
                    </a>
                </div>
                @if ($memory_quizzes->isNotEmpty())
                <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400 table-fixed">
                    ...
                    <tbody>
                        @foreach ($memory_quizzes as $quiz)
                            <tr class="bg-white border-b dark:bg-gray-900 dark:border-gray-700">
                                ...
                                <td class="px-6 py-4 hidden sm:table-cell">
                                    {{ $quiz->memory_cards_count }}
                                </td>
                                <td class="px-6 py-4 hidden sm:table-cell">
                                    {{ $quiz->only_date }}
                                </td>
                                <td
                                    class="px-6 py-4 font-bold {{ $quiz->is_valid ? 'text-green-500' : 'text-red-500' }} hidden sm:table-cell">
                                    {{ $quiz->is_valid ? trans('Yes') : trans('No') }}
                                </td>
                                <td class="px-6 py-4 grid grid-cols-3 gap-2 justify-items-center">
                                    <a href="{{ route('memory_quizzes.edit', ['tenant' => tenant('id'), 'memory_quiz' => $quiz]) }}"
                                        class="font-medium text-blue-600 dark:text-blue-500 hover:underline">
                                        {{ __('Edit') }}
                                    </a>
                                    <a href="{{ route('panel.export_user_interactions', ['tenant' => tenant('id'), 'table_name' => $quiz->table_name, 'model_id' => $quiz->id]) }}"
                                        class="font-medium text-blue-600 dark:text-blue-500 hover:underline">
                                        {{ __('Export') }}
                                    </a>
                                    <form method="post" action="{{ route('memory_quizzes.destroy', ['tenant' => tenant('id'), 'memory_quiz' => $quiz]) }}">
                                        @csrf
                                        @method('delete')
