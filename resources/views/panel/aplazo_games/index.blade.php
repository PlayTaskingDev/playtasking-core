        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="relative overflow-x-auto rounded-lg mx-5">
                ...
                <div class="grid justify-items-end">
                    <a href="{{ route('aplazo_games.create', ['tenant' => tenant('id')]) }}"
                        ...
                        {{ __('Create') }} {{ __('Aplazo game') }}
                    </a>
                </div>
                @if ($aplazo_games->isNotEmpty())
                    ...
                    <tbody>
                        @foreach ($aplazo_games as $aplazo_game)
                            <tr class="bg-white border-b dark:bg-gray-900 dark:border-gray-700">
                                ...
                                <td class="px-6 py-4 hidden sm:table-cell">
                                    {{ $aplazo_game->only_date }}
                                </td>
                                <td
                                    class="px-6 py-4 font-bold {{ $aplazo_game->is_valid ? 'text-green-500' : 'text-red-500' }} hidden sm:table-cell">
                                    {{ $aplazo_game->is_valid ? trans('Yes') : trans('No') }}
                                </td>
                                <td class="px-6 py-4 grid grid-cols-3 gap-2 justify-items-center">
                                    <a href="{{ route('aplazo_games.edit', ['tenant' => tenant('id'), 'aplazo_game' => $aplazo_game]) }}"
                                        class="font-medium text-blue-600 dark:text-blue-500 hover:underline">
                                        {{ __('Edit') }}
                                    </a>
                                    <a href="{{ route('panel.export_user_interactions', ['tenant' => tenant('id'), 'model_id' => $aplazo_game->id]) }}"
                                        class="font-medium text-blue-600 dark:text-blue-500 hover:underline">
                                        {{ __('Export') }}
                                    </a>
                                    <form method="post" action="{{ route('aplazo_games.destroy', ['tenant' => tenant('id'), 'aplazo_game' => $aplazo_game]) }}">
                                        @csrf
                                        @method('delete')
