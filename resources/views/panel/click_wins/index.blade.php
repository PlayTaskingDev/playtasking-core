        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="relative overflow-x-auto rounded-lg mx-5">
                ...
                <div class="grid justify-items-end">
                    <a href="{{ route('click_wins.create', ['tenant' => tenant('id')]) }}"
                        ...
                        {{ __('Create') }} {{ __('Click and Win') }}
                    </a>
                </div>
                @if ($click_wins->isNotEmpty())
                        ...
                        @foreach ($click_wins as $click_win)
                            <tr class="bg-white border-b dark:bg-gray-900 dark:border-gray-700">
                                ...
                                <td class="px-6 py-4 hidden sm:table-cell">
                                    {{ $click_win->description }}
                                </td>
                                <td class="px-6 py-4 hidden sm:table-cell">
                                    {{ $click_win->only_date }}
                                </td>
                                <td
                                    class="px-6 py-4 font-bold {{ $click_win->is_valid ? 'text-green-500' : 'text-red-500' }} hidden sm:table-cell">
                                    {{ $click_win->is_valid ? trans('Yes') : trans('No') }}
                                </td>
                                <td class="px-6 py-4 grid grid-cols-3 gap-2 justify-items-center">
                                    <a href="{{ route('click_wins.edit', ['tenant' => tenant('id'), 'click_win' => $click_win]) }}"
                                        class="font-medium text-blue-600 dark:text-blue-500 hover:underline">
                                        {{ __('Edit') }}
                                    </a>
                                    <a href="{{ route('panel.export_user_interactions', ['tenant' => tenant('id'), 'model_id' => $click_win->id]) }}"
                                        class="font-medium text-blue-600 dark:text-blue-500 hover:underline">
                                        {{ __('Export') }}
                                    </a>
                                    <form method="post" action="{{ route('click_wins.destroy', ['tenant' => tenant('id'), 'click_win' => $click_win]) }}">
                                        @csrf
                                        @method('delete')
