        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="relative overflow-x-auto shadow-md rounded-lg mx-5">
                ...
                <div class="grid justify-items-end">
                    <a href="{{ route('code.create', ['tenant' => tenant('id')]) }}"
                    ...
                    <tbody>
                        @foreach ($codes as $code)
                            <tr class="bg-white border-b dark:bg-gray-900 dark:border-gray-700">
                                ...
                                <td
                                    class="px-6 py-4 font-bold {{ $code->active ? 'text-green-500' : 'text-red-500' }} hidden sm:table-cell">
                                    {{ $code->active ? trans('Yes') : trans('No') }}
                                </td>
                                <td class="px-6 py-4 grid grid-cols-3 gap-2 justify-items-center">
                                    <a href="{{ route('code.edit', ['tenant' => tenant('id'), 'code' => $code]) }}"
                                        class="font-medium text-blue-600 dark:text-blue-500 hover:underline">{{ __('Edit') }}
                                    </a>
                                    <a href="{{ route('panel.export_user_interactions', ['tenant' => tenant('id'), 'table_name' => $code->table_name, 'model_id' => $code->id]) }}"
                                        class="font-medium text-blue-600 dark:text-blue-500 hover:underline">
                                        {{ __('Export') }}
                                    </a>
                                    <form method="post" action="{{ route('code.destroy', ['tenant' => tenant('id'), 'code' => $code]) }}">
                                        @csrf
                                        @method('delete')
