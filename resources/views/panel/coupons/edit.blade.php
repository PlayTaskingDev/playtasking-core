<x-panel-layout>
    <x-slot name="title">
        {{ !is_null($code->title) ? $code->title : trans('Create') . ' ' . trans('Coupon') }}
    </x-slot>
    <x-slot name="description">
        {{ $code->id == null ? '' : $code->description }}
    </x-slot>

    <x-slot name="header">
        <h1 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
            {{ $code->id == null ? trans('Create') : trans('Edit') }} {{ __('Code Hunter') }}
        </h1>
    </x-slot>
    ...
    <div class="py-6 mx-5">
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8 bg-white p-3 rounded shadow">
            <form method="POST" enctype="multipart/form-data"
                action="{{ $code->id == null ? route('code.store', ['tenant' => tenant('id')]) : route('code.update', ['tenant' => tenant('id'), 'code' => $code]) }}">
                @csrf
                @isset($code->id)
                    @method('PATCH')
                    ...
                    <select id="campaign_id" class="border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" name="campaign_id" required autofocus>
                        ...
                        @if ($campaigns)
                            @foreach ($campaigns as $campaign)
                                <option value="{{$campaign->id}}" {{(old('campaign_id') == $campaign->id ? 'selected' : ($code->campaign_id == $campaign->id ? 'selected' : ''))}}>{{$campaign->name}}</option>
                            @endforeach
                        @endif
                    </select>
                ...
                <div class="my-5">
                    <x-input-label for="type" :value="__('Code type')" />
                    <select id="type" class="border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" name="type" required autofocus>
                        <option value="">{{__('Select')}}</option>
                        @if ($code_types)
                            @foreach ($code_types as $code_type)
                                <option value="{{$code_type}}" {{(old('type') == $code_type ? 'selected' : ($code->type == $code_type ? 'selected' : ''))}}>{{$code_type}}</option>
                            @endforeach
                        @endif
                    </select>
                ...
                <div class="my-5">
                    ...
                    <x-text-input id="title" class="block mt-1 w-full" type="text" name="title"
                        :value="old('title', $code->title)" required autofocus autocomplete="title" />
                ...
                <div class="my-5">
                    ...
                    <x-text-input id="points" class="block mt-1 w-full" type="text" name="points"
                        :value="old('points', $code->points)" required autofocus autocomplete="points" />
                    <x-input-error :messages="$errors->get('points')" class="mt-2" />
                </div>

                <div class="my-5">
                    <input id="active" name="active" type="checkbox" value="1" {{$code->active ? 'checked' : ''}}
                ...
                <div class="my-5">
                    <div>
                        ...
                        @if (!is_null($code->id) && $code->featured_image)
                            <img src="{{$code->featured_image}}" alt="{{__('Code Featured Image')}}" title="{{__('Code Featured Image')}}" class="my-5">
                        @endif
                        <input
                ...
                <div class="my-5 grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <x-text-input id="gradient_1" class="block mt-1 w-full" type="text" name="gradient_1"
                            :value="old('gradient_1', $code->gradient_1)" required autofocus autocomplete="gradient_1" />
                    ...
                    <div>
                        ...
                        <x-text-input id="gradient_2" class="block mt-1 w-full" type="text" name="gradient_2"
                            :value="old('gradient_2', $code->gradient_2)" required autofocus autocomplete="gradient_2" />
                ...
                <div class="my-5 grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <x-text-input id="btn_background_color_1" class="block mt-1 w-full" type="text" name="btn_background_color_1"
                            :value="old('btn_background_color_1', $code->btn_background_color_1)" required autofocus autocomplete="btn_background_color_1" />
                    ...
                    <div>
                        ...
                        <x-text-input id="btn_background_color_2" class="block mt-1 w-full" type="text" name="btn_background_color_2"
                            :value="old('btn_background_color_2', $code->btn_background_color_2)" required autofocus autocomplete="btn_background_color_2" />
                ...
                <div class="my-5 grid grid-cols-2 gap-4">
                    <div>
                        <input id="btn_border" name="btn_border" type="checkbox" value="1" {{$code->btn_border ? 'checked' : ''}}
                            class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600">
                        ...
                        <div class="mt-3">
                            ...
                            <x-text-input id="btn_border_color" class="block mt-1 w-full" type="text" name="btn_border_color"
                                :value="old('btn_border_color', $code->btn_border_color)" required autofocus autocomplete="btn_border_color" />
                    ...
                    <div>
                        <input id="btn_shadow" name="btn_shadow" type="checkbox" value="1" {{$code->btn_shadow ? 'checked' : ''}}
                            class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600">
                        ...
                        <div class="mt-3">
                            ...
                            <x-text-input id="btn_text_color" class="block mt-1 w-full" type="text" name="btn_text_color"
                                :value="old('btn_text_color', $code->btn_text_color)" required autofocus autocomplete="btn_text_color" />
                ...
                <div class="my-5 grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <x-text-input id="btn_text_active" class="block mt-1 w-full" type="text" name="btn_text_active"
                            :value="old('btn_text_active', $code->btn_text_active)" required autofocus autocomplete="btn_text_active" />
                    ...
                    <div>
                        ...
                        <x-text-input id="btn_text_inactive" class="block mt-1 w-full" type="text" name="btn_text_inactive"
                            :value="old('btn_text_inactive', $code->btn_text_inactive)" required autofocus autocomplete="btn_text_inactive" />
                ...
                <div class="my-5 grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input id="init_date" name="init_date" value="{{ old('init_date', $code->init_date) }}" type="text" 
                    ...
                    <div>
                        ...
                        <input id="end_date" name="end_date" value="{{ old('end_date', $code->end_date) }}" 
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full pl-10 p-2.5  dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                            placeholder="{{ __('Select date') }}">
        ...
        <div class="py-6 mx-5">
            <div class="max-w-2xl mx-auto sm:px-6 lg:px-8 bg-white p-3 rounded shadow">
                <div class="flex justify-between">
                    ...
                    @if (is_null($code->award))
                        <a href="{{ route('awards.create', ['tenant' => tenant('id'), 'awardable_id' => $code, 'awardable_type' => 'App\Models\Code']) }}"
                        ...
                        <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                            ...
                            <tbody>
                                <tr class="bg-white border-b dark:bg-gray-900 dark:border-gray-700">
                                    ...
                                    <td class="px-6 py-4">
                                        <a href="{{ route('awards.edit', ['tenant' => tenant('id'), 'award' => $code->award]) }}"
        ...
        <script>
            document.addEventListener("DOMContentLoaded", () => {
                ...
                const gradient3 = new ColorPicker({
                    color: '{{$code->btn_background_color_1}}',
                    background: '#fff',
                ...
                const gradient4 = new ColorPicker({
                    color: '{{$code->btn_background_color_2}}',
                    background: '#fff',
