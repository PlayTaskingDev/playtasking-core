        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="relative overflow-x-auto rounded-lg mx-5">
                ...
                <div class="grid justify-items-end">
                    <a href="{{ route('panel.vote_contest.create', ['tenant' => tenant('id')]) }}"
                        ...
                        {{ __('Create') }} {{ __('Vote Contest') }}
                    </a>
                </div>
                @if ($vote_contests->isNotEmpty())
                <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400 table-fixed">
                    ...
                    <tbody>
                        @foreach ($vote_contests as $vote_contest)
                            <tr class="bg-white border-b dark:bg-gray-900 dark:border-gray-700">
                                ...
                                <td class="px-6 py-4 hidden sm:table-cell">
                                    {{ $vote_contest->only_date }}
                                </td>
                                <td
                                    class="px-6 py-4 font-bold {{ $vote_contest->is_valid ? 'text-green-500' : 'text-red-500' }} hidden sm:table-cell">
                                    {{ $vote_contest->is_valid ? trans('Yes') : trans('No') }}
                                </td>
                                <td class="px-6 py-4 grid grid-cols-3 gap-2 justify-items-center">
                                    <a href="{{ route('panel.vote_contest.edit', ['tenant' => tenant('id'), 'vote_contest' => $vote_contest]) }}"
                                    ...
                                    <form method="post" action="{{ route('panel.vote_contest.destroy', ['tenant' => tenant('id'), 'vote_contest' => $vote_contest]) }}">
                                        @csrf
                                        @method('delete')
