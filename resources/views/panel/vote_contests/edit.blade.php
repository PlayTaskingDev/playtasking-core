    ...
    <div class="py-6 mx-5">
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8 bg-white p-3 rounded shadow">
            <form method="POST" enctype="multipart/form-data"
                action="{{ $vote_contest->id == null ? route('panel.vote_contest.store', ['tenant' => tenant('id')]) : route('panel.vote_contest.update', ['tenant' => tenant('id'), 'vote_contest' => $vote_contest]) }}">
                @csrf
                    ...
                    <select id="campaign_id" class="border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" name="campaign_id" required autofocus>
                        ...
                        @if ($campaigns)
                            @foreach ($campaigns as $campaign)
                                <option value="{{$campaign->id}}" {{(old('campaign_id') == $campaign->id ? 'selected' : (isset($vote_contest->campaign) && $vote_contest->campaign->id == $campaign->id ? 'selected' : ''))}}>{{$campaign->name}}</option>
                            @endforeach
                        @endif
                    </select>
                ...
                <div class="my-5 grid grid-cols-3 gap-4">
                    <div>
                        ...
                        <select id="asset_type" class="border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" name="asset_type" required autofocus>
                            <option value="">{{__('Select')}}</option>
                            <option value="photo" {{old('asset_type') == 'photo' ? 'selected' : ($vote_contest->id && $vote_contest->asset_type == 'photo' ? 'selected' : '')}}>{{__('Photo')}}</option>
                            <option value="video" {{old('asset_type') == 'video' ? 'selected' : ($vote_contest->id && $vote_contest->asset_type == 'video' ? 'selected' : '')}}>{{__('Video')}}</option>
                        </select>
                        <x-input-error :messages="$errors->get('campaign_id')" class="mt-2" />
                    </div>
                    <div>
                        <x-input-label for="asset_kb_size" :value="__('Asset size (Mb)')" />
                        <x-text-input id="asset_kb_size" class="block mt-1 w-full" type="number" name="asset_kb_size"
                            :value="old('asset_kb_size', $vote_contest->mb_size)" required autofocus autocomplete="asset_kb_size" />
                        <x-input-error :messages="$errors->get('asset_kb_size')" class="mt-2" />
                    </div>
                    <div>
                        <x-input-label for="points_per_vote" :value="__('Points per vote')" />
                        <x-text-input id="points_per_vote" class="block mt-1 w-full" type="number" name="points_per_vote"
                            :value="old('points_per_vote', $vote_contest->points_per_vote)" required autofocus autocomplete="points_per_vote" />
                        <x-input-error :messages="$errors->get('points_per_vote')" class="mt-2" />
                    </div>
                </div>
                ...
                <div class="my-5">
                    ...
                    <x-text-input id="title" class="block mt-1 w-full" type="text" name="title"
                        :value="old('title', $vote_contest->title)" required autofocus autocomplete="title" />
                ...
                <div class="my-5">
                    ...
                    <x-text-input id="description" class="block mt-1 w-full" type="text" name="description"
                        :value="old('description', $vote_contest->description)" required autofocus autocomplete="description" />
                ...
                <div class="my-5 grid grid-cols-2 gap-4">
                    <div>
                        ...
                        @if (!is_null($vote_contest->id) && $vote_contest->featured_image)
                            <img src="{{$vote_contest->featured_image}}" alt="{{__('Vote Contest Featured Image')}}" title="{{__('Vote Contest Featured Image')}}" class="my-5">
                        @endif
                        <input
                    ...
                    <div>
                        ...
                        @if (!is_null($vote_contest->id) && $vote_contest->featured_image_disabled)
                            <img src="{{$vote_contest->featured_image_disabled}}" alt="{{__('Vote Contest Disabled Image')}}" title="{{__('Vote Contest Disabled Image')}}" class="my-5">
                        @endif
                        <input
                            class="block w-full text-sm text-gray-900 border border-gray-300 rounded-lg cursor-pointer bg-gray-50 dark:text-gray-400 focus:outline-none dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400"
                ...
                <div class="my-5">
                    ...
                    <x-text-input id="slug" class="block mt-1 w-full" type="text" name="slug"
                        :value="old('slug', $vote_contest->slug)" required autofocus autocomplete="slug" />
                ...
                <div class="my-5 grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <x-text-input id="gradient_1" class="block mt-1 w-full" type="text" name="gradient_1"
                            :value="old('gradient_1', $vote_contest->gradient_1)" required autofocus autocomplete="gradient_1" />
                    ...
                    <div>
                        ...
                        <x-text-input id="gradient_2" class="block mt-1 w-full" type="text" name="gradient_2"
                            :value="old('gradient_2', $vote_contest->gradient_2)" required autofocus autocomplete="gradient_2" />
                ...
                <div class="my-5 grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <x-text-input id="btn_background_color_1" class="block mt-1 w-full" type="text" name="btn_background_color_1"
                            :value="old('btn_background_color_1', $vote_contest->btn_background_color_1)" required autofocus autocomplete="btn_background_color_1" />
                    ...
                    <div>
                        ...
                        <x-text-input id="btn_background_color_2" class="block mt-1 w-full" type="text" name="btn_background_color_2"
                            :value="old('btn_background_color_2', $vote_contest->btn_background_color_2)" required autofocus autocomplete="btn_background_color_2" />
                ...
                <div class="my-5 grid grid-cols-2 gap-4">
                    <div>
                        <input id="btn_border" name="btn_border" type="checkbox" value="1" {{$vote_contest->btn_border ? 'checked' : ''}}
                        ...
                        <div class="mt-3">
                            ...
                            <x-text-input id="btn_border_color" class="block mt-1 w-full" type="text" name="btn_border_color"
                                :value="old('btn_border_color', $vote_contest->btn_border_color)" required autofocus autocomplete="btn_border_color" />
                    ...
                    <div>
                        <input id="btn_shadow" name="btn_shadow" type="checkbox" value="1" {{$vote_contest->btn_shadow ? 'checked' : ''}}
                            class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600">
                        ...
                        <div class="mt-3">
                            ...
                            <x-text-input id="btn_text_color" class="block mt-1 w-full" type="text" name="btn_text_color"
                                :value="old('btn_text_color', $vote_contest->btn_text_color)" required autofocus autocomplete="btn_text_color" />
                ...
                <div class="my-5 grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <x-text-input id="btn_text_active" class="block mt-1 w-full" type="text" name="btn_text_active"
                            :value="old('btn_text_active', $vote_contest->btn_text_active)" required autofocus autocomplete="btn_text_active" />
                    ...
                    <div>
                        ...
                        <x-text-input id="btn_text_inactive" class="block mt-1 w-full" type="text" name="btn_text_inactive"
                            :value="old('btn_text_inactive', $vote_contest->btn_text_inactive)" required autofocus autocomplete="btn_text_inactive" />
                ...
                <div class="my-5">
                    ...
                    @if ($vote_contest->game_banner)
                        <div id="delete_image_holder" class="relative">
                            <img src="{{$vote_contest->game_banner}}" alt="{{__('Banner Image')}}" title="{{__('Banner Image')}}" class="my-5 w-full">
                ...
                <div class="my-5">
                    ...
                    <x-text-input id="game_banner_url" class="block mt-1 w-full" type="text" name="game_banner_url"
                        :value="old('game_banner_url', $vote_contest->game_banner_url)" autofocus autocomplete="game_banner_url" />
                    <x-input-error :messages="$errors->get('game_banner_url')" class="mt-2" />
                </div>
                <div class="my-5">
                    <x-input-label for="game_banner_video" :value="__('Video')" />
                    @if (!is_null($vote_contest->id) && $vote_contest->game_banner_video)
                    ...
                    <x-text-input id="game_banner_video" class="block mt-1 w-full" type="text" name="game_banner_video"
                        :value="old('game_banner_video', $vote_contest->game_banner_video)" autofocus autocomplete="game_banner_video" />
                ...
                <div class="my-5 grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input id="init_date" name="init_date" value="{{ old('init_date', $vote_contest->init_date) }}" type="text" 
                    ...
                    <div>
                        ...
                        <input id="end_date" name="end_date" value="{{ old('end_date', $vote_contest->end_date) }}" 
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full pl-10 p-2.5  dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                            placeholder="{{ __('Select date') }}">
        ...
        <script>
            document.addEventListener("DOMContentLoaded", () => {
                ...
                const gradient3 = new ColorPicker({
                    color: '{{$vote_contest->btn_background_color_1}}',
                    background: '#fff',
                ...
                const gradient4 = new ColorPicker({
                    color: '{{$vote_contest->btn_background_color_2}}',
                    background: '#fff',
                ...
                const gradient5 = new ColorPicker({
                    color: '{{$vote_contest->btn_border_color}}',
                    background: '#fff',
                ...
                const gradient6 = new ColorPicker({
                    color: '{{$vote_contest->btn_text_color}}',
                    background: '#fff',
