        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="relative overflow-x-auto rounded-lg mx-5">
                ...
                <div class="grid justify-items-end">
                    <a href="{{ route('smash_games.create', ['tenant' => tenant('id')]) }}"
                    ...
                    <tbody>
                        @foreach ($smash_games as $smash_game)
                            <tr class="bg-white border-b dark:bg-gray-900 dark:border-gray-700">
                                ...
                                <td class="px-6 py-4 hidden sm:table-cell">
                                    {{ $smash_game->only_date }}
                                </td>
                                <td
                                    class="px-6 py-4 font-bold {{ $smash_game->is_valid ? 'text-green-500' : 'text-red-500' }} hidden sm:table-cell">
                                    {{ $smash_game->is_valid ? trans('Yes') : trans('No') }}
                                </td>
                                <td class="px-6 py-4 grid grid-cols-3 gap-2 justify-items-center">
                                    <a href="{{ route('smash_games.edit', ['tenant' => tenant('id'), 'smash_game' => $smash_game]) }}"
                                    ...
                                    <form method="post" action="{{ route('smash_games.destroy', ['tenant' => tenant('id'), 'smash_game' => $smash_game]) }}">
                                        @csrf
                                        @method('delete')
