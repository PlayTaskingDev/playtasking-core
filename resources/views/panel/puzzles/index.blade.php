        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="relative overflow-x-auto rounded-lg mx-5">
                ...
                <div class="grid justify-items-end">
                    <a href="{{ route('puzzles.create', ['tenant' => tenant('id')]) }}"
                    ...
                    <tbody>
                        @foreach ($puzzles as $puzzle)
                            <tr class="bg-white border-b dark:bg-gray-900 dark:border-gray-700">
                                ...
                                <td
                                    class="px-6 py-4 font-bold {{ $puzzle->is_valid ? 'text-green-500' : 'text-red-500' }} hidden sm:table-cell">
                                    {{ $puzzle->is_valid ? trans('Yes') : trans('No') }}
                                </td>
                                <td class="px-6 py-4 grid grid-cols-3 gap-2 justify-items-center">
                                    <a href="{{ route('puzzles.edit', ['tenant' => tenant('id'), 'puzzle' => $puzzle]) }}"
                                    ...
                                    <form method="post" action="{{ route('puzzles.destroy', ['tenant' => tenant('id'), 'puzzle' => $puzzle]) }}">
                                        @csrf
                                        @method('delete')
