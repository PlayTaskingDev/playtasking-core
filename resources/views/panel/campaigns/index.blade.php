        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="relative overflow-x-auto rounded-lg mx-5">
                ...
                <div class="grid justify-items-end">
                    <a href="{{ route('panel.campaign.create', ['tenant' => tenant('id')]) }}"
                    ...
                    <tbody>
                        @foreach ($campaigns as $campaign)
                            <tr class="bg-white border-b dark:bg-gray-900 dark:border-gray-700">
                                ...
                                <td class="px-6 py-4 hidden sm:table-cell">
                                    {{ $campaign->quizzes_count +  $campaign->memory_quizzes_count + $campaign->share_quizzes_count}}
                                </td>
                                <td class="px-6 py-4 hidden sm:table-cell">
                                    {{ $campaign->only_date }}
                                </td>
                                <td
                                    class="px-6 py-4 font-bold {{ $campaign->active ? 'text-green-500' : 'text-red-500' }} hidden sm:table-cell">
                                    {{ $campaign->active ? trans('Yes') : trans('No') }}
                                </td>
                                <td class="px-6 py-4 grid grid-cols-2 gap-2 justify-items-center">
                                    <a href="{{ route('panel.campaign.edit', ['tenant' => tenant('id'), 'campaign' => $campaign]) }}"
                                        class="font-medium text-blue-600 dark:text-blue-500 hover:underline">{{ __('Edit') }}</a>
                                    <form method="post" action="{{ route('panel.campaign.destroy', ['tenant' => tenant('id'), 'campaign' => $campaign]) }}">
                                        @csrf
                                        @method('delete')
