        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="relative overflow-x-auto rounded-lg mx-5">
                ...
                <div class="grid justify-items-end">
                    <a href="{{ route('quizzes.create', ['tenant' => tenant('id')]) }}"
                        ...
                        @foreach ($quizzes as $quiz)
                            <tr class="bg-white border-b dark:bg-gray-900 dark:border-gray-700">
                                ...
                                <td class="px-6 py-4 grid grid-cols-3 gap-2 justify-items-center">
                                    <a href="{{ route('quizzes.edit', ['tenant' => tenant('id'), 'quiz' => $quiz]) }}"
                                        class="font-medium text-blue-600 dark:text-blue-500 hover:underline">
                                        {{ __('Edit') }}
                                    </a>
                                    <a href="{{ route('panel.export_user_interactions', ['tenant' => tenant('id'), 'model_id' => $quiz->id]) }}"
                                        class="font-medium text-blue-600 dark:text-blue-500 hover:underline">
                                        {{ __('Export') }}
                                    </a>
                                    <form method="post" action="{{ route('quizzes.destroy', ['tenant' => tenant('id'), 'quiz' => $quiz]) }}">
                                        @csrf
                                        @method('delete')
